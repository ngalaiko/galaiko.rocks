...
RUN \
    ...
    && mkdir -p "/tmp/reproxy_v${REPROXY_VERSION}" \
    && tar -C "/tmp/reproxy_v${REPROXY_VERSION}" -xzf "/tmp/reproxy_v${REPROXY_VERSION}_linux_arm64.tar.gz" \
    && mv "/tmp/reproxy_v${REPROXY_VERSION}/reproxy" /usr/bin/reproxy \
    \
    && rm -rf "/tmp/*"
