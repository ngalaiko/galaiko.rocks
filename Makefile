...
$(BUILD_DIR)/movies/index.html: $(INPUT_MOVIE_FILES) $(call templ,movies/index.html.jinja)
	...
	@mkdir -p "$(dir $@)"
	@cat $(INPUT_MOVIE_FILES) | $(JQ_BIN) --slurp '{ entries: . }' | $(J2_BIN) movies/index.html.jinja > "$@" || exit 1
...
$(BUILD_DIR)/places/index.html: $(INPUT_PLACE_FILES) $(call templ,places/index.html.jinja)
	...
	@mkdir -p "$(dir $@)"
	@cat $(INPUT_PLACE_FILES) | $(JQ_BIN) --slurp '{ places: . }' | $(J2_BIN) places/index.html.jinja > "$@" || exit 1

# Cocktails
$(BUILD_DIR)/cocktails/index.html: $(INPUT_COCKTAIL_FILES)
	@echo '$(SRC_DIR)/cocktails/*.cook -> $@'
	@mkdir -p "$(dir $@)"
	@ls $(INPUT_COCKTAIL_FILES) | xargs -I {} $(COOK_BIN) recipe read --format json {} | $(JQ_BIN) --slurp '{ cocktails: . }' | $(J2_BIN) cocktails/index.html.jinja > "$@" || exit 1
...
$(BUILD_DIR)/records/index.html: $(INPUT_RECORD_FILES) $(call templ,records/index.html.jinja)
	...
	@mkdir -p "$(dir $@)"
	@cat $(INPUT_RECORD_FILES) | $(JQ_BIN) --slurp '{ records: . }' | $(J2_BIN) records/index.html.jinja > "$@" || exit 1
...
$(BUILD_DIR)/posts/index.html: $(INPUT_POST_FILES) $(call templ,posts/index.html.jinja)
	...
	@mkdir -p "$(dir $@)"
	@ls $(INPUT_POST_FILES) | xargs -I {} sh -c './scripts/convert_md.sh {} | $(YQ_BIN) --output-format json' | $(JQ_BIN) --slurp '{ posts: . }' | $(J2_BIN) posts/index.html.jinja > "$@" || exit 1

$(BUILD_DIR)/posts/index.xml: $(INPUT_POST_FILES) $(call templ,posts/index.xml.jinja)
	@echo '$(SRC_DIR)/posts/*.md -> $@'
	@mkdir -p "$(dir $@)"
	@ls $(INPUT_POST_FILES) | xargs -I {} sh -c './scripts/convert_md.sh {} | $(YQ_BIN) --output-format json' | $(JQ_BIN) --slurp '{ posts: . }' | $(J2_BIN) posts/index.xml.jinja > "$@" || exit 1

$(BUILD_DIR)/posts/index.atom: $(INPUT_POST_FILES) $(call templ,posts/index.atom.jinja)
	@echo '$(SRC_DIR)/posts/*.md -> $@'
	@mkdir -p "$(dir $@)"
	@ls $(INPUT_POST_FILES) | xargs -I {} sh -c './scripts/convert_md.sh {} | $(YQ_BIN) --output-format json' | $(JQ_BIN) --slurp '{ posts: . }' | $(J2_BIN) posts/index.atom.jinja > "$@" || exit 1
